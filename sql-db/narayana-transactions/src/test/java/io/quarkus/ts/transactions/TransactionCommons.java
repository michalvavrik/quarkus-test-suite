        ...
        try {
            ...
            if (isBareMetalPlatform()) {
                ...
                if (!isDatabaseTableLockedWhenTransactionFailed()) {
                    ...
                    getApp().logs().assertContains("XARecoveryModule state change FIRST_PASS->BETWEEN_PASSES",
                            "Datasource xa-ds-2 enables XA but transaction recovery is not enabled");
                    getApp().logs().assertContains("first pass processed",
